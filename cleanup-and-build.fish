...
echo -e "${YELLOW}[CLEAN] Cleaning up temp-tracker...${NC}"
...
if test -f ./temp-tracker
    ...
    echo -e "${YELLOW}[WARN] No binary found to remove${NC}"
end
...
if test -n "$pids"
    echo -e "${YELLOW}[FIND] Found running temp-tracker process(es): $pids${NC}"
    ...
    for pid in $pids
        ...
        if kill -0 $pid 2>/dev/null
            echo -e "${RED}  [WARN] PID $pid still running, forcing kill...${NC}"
            kill -9 $pid 2>/dev/null  # SIGKILL — force terminate, like Python's process.terminate()
        else
            echo -e "${GREEN}  [OK] PID $pid stopped${NC}"
        end
    end
else
    echo -e "${GREEN}[OK] No running temp-tracker processes found${NC}"
end

# Step 3: Rebuild the binary
echo -e "${YELLOW}[BUILD] Building temp-tracker...${NC}"
...
echo ""
echo -e "${GREEN}[DONE] Cleanup and rebuild complete!${NC}"